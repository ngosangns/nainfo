...
function getNonIgnoredFilePaths() {
    return new Promise((resolve, reject) => {
        exec('git ls-files --cached --others --exclude-standard', (error, stdout, stderr) => {
            if (error) {
                reject(error);
                return;
            }
...
async function main() {
    ...
    const excludeFiles = ['go.mod', 'go.sum', '.git', 'project-content.js', 'README.md', 'api.md', 'architecture.md', 'env.example', '.gitignore', '.air.toml', 'profile.pb.go', 'profile_grpc.pb.go'];
    let output = '';
